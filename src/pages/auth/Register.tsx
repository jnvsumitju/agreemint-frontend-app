...
export function Register() {
  ...
  function validateField(field: string, value: string, ctx?: { password?: string; confirmPassword?: string }) {
    setErrors((prev) => {
      ...
      switch (field) {
        case 'name':
          next.name = value.trim().length < 2 ? 'Name must be at least 2 characters' : undefined
          break
        case 'email':
          next.email = value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)
    ...
    <AuthLayout>
      <div className="page-enter">
        ...
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          {inviteToken ? 'Sign up to accept your invitation' : 'Start building amazing templates for free'}
        </p>
          ...
          <>
            <div className="mt-6 flex gap-3">
              {providers?.google && (
                <a
                  ...
                  className="flex flex-1 items-center justify-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-medium text-zinc-700 shadow-sm transition-colors hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                >
              ...
              {providers?.github && (
                <a
                  href="/api/oauth2/authorize/github"
                  className="flex flex-1 items-center justify-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-medium text-zinc-700 shadow-sm transition-colors hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                >
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-zinc-50 px-3 text-zinc-500 dark:bg-zinc-950 dark:text-zinc-400">
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className={hasOAuth ? '' : 'mt-6'}>
          {errors.form && (
        ...
        <p className="mt-6 text-center text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <Link
            to="/login"
            className="font-medium text-violet-600 hover:text-violet-700 hover:underline dark:text-violet-400"
          >
            Sign in
          </Link>
        </p>
